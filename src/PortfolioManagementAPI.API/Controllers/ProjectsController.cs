...
public class ProjectsController : ControllerBase
{
    ...
    public async Task<ActionResult<PagedResponse<ProjectResponse>>> GetProjects([FromQuery] PaginationParams paginationParams)
    {
        ...
        var projectDtos = _mapper.Map<List<ProjectResponse>>(projects);

        var query = projectDtos.AsQueryable();
        ...
        if (!string.IsNullOrWhiteSpace(paginationParams.SortBy))
        {
            ...
            {
                "name" => paginationParams.IsDescending ? query.OrderByDescending(p => p.Name) : query.OrderBy(p => p.Name),
                "createdat" => paginationParams.IsDescending ? query.OrderByDescending(p => p.CreatedAt) : query.OrderBy(p => p.CreatedAt),
                "likecount" => paginationParams.IsDescending ? query.OrderByDescending(p => p.LikeCount) : query.OrderBy(p => p.LikeCount),
                _ => query
            };
        }
